---
title: "Hybrid objective model (IDOCRIW-CoCoSo) optimizes groundwater potential"
excerpt: "Graphical abstract <br/><img src='soliu-mudashiru/images/portfolio/Idocriw_cocoso/1.PNG'>"
collection: portfolio
---

**Keywords** Groundwater, Objective MCDM algorithm, Sustainability. 

**Description** Under review manuscript (2025). 

### Highlights 
- Applied Integrated Determination of Objective Criteria Weights-Combined Compromise Solution (IDOCRIW-CoCoSo) multi-criteria decision-making approach.

- Utilized remote sensing and geophysical conditioning datasets for the modelling.

- Used other objective models (CRITIC-CoCoSo; Mean Weighted CoCoSo) as comparative tools. 

- The produced groundwater potential map (IDOCRIW-CoCoSo) is advantageous in meeting SDG 6.1 goals

<figure>
  <img src= "soliu-mudashiru/images/portfolio/Idocriw_cocoso/2.PNG" alt="Groundwater Potential Map (IDOCRIW-CoCoSo)">
  <figcaption><em>Groundwater Potential Map (IDOCRIW-CoCoSo)</em></figcaption>
</figure>
<figure>
  <img src="soliu-mudashiru/images/portfolio/Idocriw_cocoso/3.PNG" alt="Groundwater Potential Map (CRITIC-CoCoSo)">
  <figcaption><em>Groundwater Potential Map (CRITIC-CoCoSo)</em></figcaption>
</figure>
<figure>
  <img src="soliu-mudashiru/images/portfolio/Idocriw_cocoso/4.PNG" alt="Groundwater Potential Map (Mean Weighted-CoCoSo)">
  <figcaption><em>Groundwater Potential Map (Mean Weighted-CoCoSo)</em></figcaption>
</figure> 

### Methodology ### 
- Creation of conditioning factors maps using ArcGIS 10.8.2 software. 

- Fishnet points extraction into Python programming coded models. 

- Groundwater potential maps based on the models. 

- Reliability and comparative 

<figure>
  <img src="soliu-mudashiru/images/portfolio/Idocriw_cocoso/5.PNG" alt="Methodology Flowchart">
  <figcaption><em>Methodology Flowchart</em></figcaption>
</figure>

### Findings 
- Potentiality indices varied significantly across the location in the study area. 

- GIS integration improved spatial accuracy. 

- The methodology is adaptable to other regions with similar geologic context.
 
- IDOCRIW-CoCoSo demonstrated stability during scenario analysis

<figure>
  <img src="soliu-mudashiru/images/portfolio/Idocriw_cocoso/7.PNG" alt="Sensitivity analysis of IDOCRIW-CoCoSo">
  <figcaption><em>Sensitivity analysis of IDOCRIW-CoCoSo</em></figcaption>
</figure>


